...
public final class NetflixCSVDataModel implements DataModel {
  ...
  private static final Logger log = LoggerFactory.getLogger(NetflixCSVDataModel.class);
   ...
   * @throws IOException
   */
  public NetflixCSVDataModel(File dataDirectory, boolean useSubset) throws IOException {
  ...
  private FastByIDMap<PreferenceArray> readUsers(File dataDirectory) throws IOException {
    ...
    File netflixFile = new File(dataDirectory,"netflix-dataset-wodates.csv");
    Iterator<String> lineIterator = new FileLineIterable(netflixFile,false).iterator();
    while(lineIterator.hasNext()) {
      ...
      long userID = Long.parseLong(line.substring(firstComma+1,secondComma));
      float rating = Float.parseFloat(line.substring(secondComma, line.length()-1));
