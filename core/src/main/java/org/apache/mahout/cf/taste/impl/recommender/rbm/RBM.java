...
public class RBM extends AbstractJob {
  ...
  public int train() {
    ...
    for (j = 0; j < numItems; j++) {
      ...
      for (i = 0; i < softmax; i++) {
        visbiases[j][i] = Math.log(((double) moviercount[j * softmax + i])
            / ((double) mtot));
      }
    }
    ...
    while (((nrmse < (last_rmse - e)) || loopcount < 14) && loopcount < 80) {
      ...
      if (loopcount > 5) Momentum = finalMomentum;
      ...
      for (u = 0; u < numUsers; u++) {
        ...
        do {
          ...
          for (j = 0; j < count; j++) {
            ...
            for (i = 0; i < 5; i++) {
              negvisprobs[m][i] = 1. / (1 + Math.exp(-negvisprobs[m][i]
                  - visbiases[m][i]));
            }
            ...
            if ((randval -= negvisprobs[m][0]) <= 0.0) negvissoftmax[m] = 0;
            else if ((randval -= negvisprobs[m][1]) <= 0.0) negvissoftmax[m] = 1;
            else if ((randval -= negvisprobs[m][2]) <= 0.0) negvissoftmax[m] = 2;
            else if ((randval -= negvisprobs[m][3]) <= 0.0) negvissoftmax[m] = 3;
            else /** The case when ((randval -= negvisprobs[m][4]) <= 0.0) */
            negvissoftmax[m] = 4;
            
            /** If in training data, then train on it */
            if (j < d0 && finalTStep) negvisact[m][negvissoftmax[m]] += 1.0;
          }
          ...
          if (stepT == 0) {
            ...
            for (j = 0; j < d0; j++) {
              ...
              double expectedV = nvp2[m][1] + 2.0 * nvp2[m][2] + 3.0
                  * nvp2[m][3] + 4.0 * nvp2[m][4];
        ...
        for (j = 0; j < d0; j++) {
          ...
          for (h = 0; h < totalFeatures; h++) {
            ...
            CDneg[m][negvissoftmax[m]][h] += neghidstates[h];
          }
        }
        ...
        if (((u + 1) % bSize) == 0 || (u + 1) == numUsers) {
          ...
          for (m = 0; m < numItems; m++) {
            ...
            for (rr = 0; rr < softmax; rr++) {
              if (posvisact[m][rr] != 0.0 || negvisact[m][rr] != 0.0) {
                posvisact[m][rr] /= (moviecount[m]);
                negvisact[m][rr] /= (moviecount[m]);
                visbiasinc[m][rr] = Momentum * visbiasinc[m][rr] + EpsilonVB
                    * ((posvisact[m][rr] - negvisact[m][rr]));
          ...
          for (h = 0; h < totalFeatures; h++) {
            if (poshidact[h] != 0.0 || neghidact[h] != 0.0) {
              poshidact[h] /= ((numcases));
              neghidact[h] /= ((numcases));
              hidbiasinc[h] = Momentum * hidbiasinc[h] + EpsilonHB
                  * ((poshidact[h] - neghidact[h]));
